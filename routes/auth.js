...
authRouter.get("/callback", async (req, res) => {
  ...
  const tokenResult = await axios.post(
    `${process.env.GOOGLE_TOKEN_ENDPOINT}?${QueryString.stringify(queryParams)}`
  );
  if (tokenResult.data) {
  ...
  try {
    ...
    if (!user) {
      ...
      res.status(200).json({
        success: true,
        isNewUser: true,
        access_token: generateAccessToken(userDetails),
...
authRouter.post("/savePassword", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    user.password = CryptoJS.SHA1(password);
    user.encryptionKey = CryptoJS.SHA1(encryptionKey);

    await user.save();

    res
      .status(200)
      .json({ success: true, message: "Password saved successfully" });
  } catch (e) {
...
authRouter.post("/checkPassword", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    if (user.password == CryptoJS.SHA1(password)) {
